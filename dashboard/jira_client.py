...
class JiraClient:
    ...
    def fetch_bugs(self) -> dict[str, JiraResult]:
        ...
        for product in self.config["products"]:
            ...
            jql = Template(product.get("jira_jql", self.config["jira_jql"])).render(
            ...
            tickets = [
                {
                    ...
                    "status": (issue.get("fields", {}).get("status") or {}).get(
                        "name", "Unknown"
                    ),
    ...
    def _query(self, base: str, jql: str) -> Optional[Any]:
        url = (
